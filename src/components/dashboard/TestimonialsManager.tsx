  ...
  return (
    <div>
      <PageHeader
        icon="💬"
        title="آراء العملاء"
        subtitle="التقييمات (المعتمدة تظهر على الموقع)"
        actions={
          <button className="btn btn-primary" onClick={() => setEdit(blank)}>+ إضافة رأي</button>
        }
      />

      {items.length === 0 ? (
        <div className="panel" style={{ textAlign: 'center', padding: 46, color: 'var(--sub)' }}>مفيش آراء لسه.</div>
      ) : (
          ...
          {items.map((t) => (
            <div className="proj-manage-card" key={t.id}>
              <div className="pm-body" style={{ paddingTop: 16 }}>
                ...
                {!t.approved && <span style={{ color: '#f59e0b' }}>بانتظار الموافقة</span>}
              </div>
        ...
        <div className="modal-overlay" onClick={() => setEdit(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-head">
              ...
              <strong>{edit.id ? 'تعديل رأي' : 'رأي جديد'}</strong>
            </div>
            <div className="modal-body">
              ...
              <div className="grid-2">
                <div>
                  <label className="lbl">المسمى</label>
              ...
              <label className="lbl">الرأي</label>
            ...
            <div className="modal-foot">
              ...
              <button className="btn btn-primary" onClick={save} disabled={busy || !edit.name.trim() || !edit.content.trim()}>{busy ? '...' : '💾 حفظ'}</button>
            </div>
          </div>
        </div>
      )}
